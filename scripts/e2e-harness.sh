...
eh_build_and_gate() {
	...
	# Same for the v2.1.0 architecture-evidence gate (tests/prod/280-architecture-governance.sh):
	# the fixtures declare no architecture producers, so missing_architecture_evidence would
	# otherwise fire on stack shape rather than on the tool contract under test.
	_envf="$_w/reports/sentinel-shield-gates.env"
	awk '/^SENTINEL_SHIELD_FAIL_ON_(COVERAGE_THRESHOLD_VIOLATIONS|COVERAGE_REGRESSION|MUTATION_SCORE_VIOLATIONS|COMPLEXITY_VIOLATIONS|DUPLICATION_VIOLATIONS|DEAD_CODE_VIOLATIONS|MISSING_COVERAGE_EVIDENCE|CHANGED_LINES_COVERAGE_VIOLATIONS|MISSING_TEST_EVIDENCE|EMPTY_TEST_SUITE|SKIPPED_TESTS|FOCUSED_TEST_VIOLATIONS|SKIPPED_TEST_MARKER_VIOLATIONS|DEBUG_CODE_VIOLATIONS|LARGE_FILE_VIOLATIONS|LARGE_FUNCTION_VIOLATIONS|MISSING_ARCHITECTURE_EVIDENCE)=/{sub(/=.*/,"=false")}{print}' "$_envf" > "$_envf.tmp" && mv "$_envf.tmp" "$_envf"
	_rc=0

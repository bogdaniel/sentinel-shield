...
while [ "$_i" -lt "$_reqcount" ]; do
	...
	if [ "$_applicable" != "true" ]; then
		jq -n --arg n "$_name" --arg c "$_cat" --argjson v "$_verjson" \
			'{name:$n, category:$c, applicable:false, executed:false, status:"not-applicable", version:$v, database_age_days:null, fresh:true, targets_scanned:null, raw_report_digest:null}' >> "$SCAN_NDJSON"
		_i=$((_i + 1)); continue
	fi

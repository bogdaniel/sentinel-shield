...
# consumer-validation records conforming to schemas/node-consumer-validation.schema.json
# (line-delimited JSON, one object per line). Real-consumer validation drivers
# (e.g. tests/prod/201-node-consumers.sh) use it so every consumer produces
# uniform, machine-checkable evidence.
#
# Requires the shared library first (for log_* / command_exists) and jq:
#   . "$SCRIPT_DIR/lib/sentinel-shield-common.sh"
#   . "$SCRIPT_DIR/scripts/report-consumer-validation.sh"
...
rcv_validate() {
	...
	_bad=$(jq -c '
		...
		| select(
			...
			or ([ "package-manager","lockfile","one-of","typecheck","lint","test","audit","rollback","install" ] | index($r.gate) | not)
		)' "$1" 2>/dev/null) || {

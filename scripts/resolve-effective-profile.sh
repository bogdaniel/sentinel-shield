...
# Normalize an explicit override to JSON (+ schema-validate) via the override lib.
OVR_JSON=""
if [ -n "$OVERRIDE" ]; then
	[ -f "$OVERRIDE" ] || { log_error "override file not found: $OVERRIDE"; exit 2; }
	if command -v tpo_to_json >/dev/null 2>&1; then
		OVR_JSON=$(mktemp 2>/dev/null || mktemp -t ssovr)
		tpo_to_json "$OVERRIDE" > "$OVR_JSON" || { log_error "invalid tool-policy override: $OVERRIDE"; rm -f "$OVR_JSON"; exit 2; }
	else
		case "$OVERRIDE" in
			*.json) OVR_JSON="$OVERRIDE" ;;
			*) log_error "cannot parse override '$OVERRIDE' (tool-policy-override.sh absent; pass a .json override)"; exit 2 ;;
		esac
	fi
fi

...
ss_shape_or_fail "$TOOL" "$INPUT" '(type == "object") and (((.dependencies? | type) == "array") or ((.critical? | type) == "number"))' '{"critical_vulnerabilities":0,"high_vulnerabilities":0,"medium_vulnerabilities":0}'

...
# it per stack (dependency-check -> dependency_check), and several producers may legitimately share one
...
while [ $# -gt 0 ]; do case "$1" in
  ...
  -h|--help) echo "Usage: dependency-check.sh [--input <path>] [--tool-name <name>] [--producer-key <key>] [--fixture-evidence]"; exit 0 ;;
...
ENVELOPE=$(ne_envelope "$PRODUCER" "$INPUT" "dependency-check-json" "$NE_TRUST_TYPE" \

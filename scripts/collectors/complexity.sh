...
# it per stack (complexity -> complexity), and several producers may legitimately share one
...
if [ "$V" = "missing" ]; then
	...
	ss_emit_collector "$TOOL" "execution-error" \
		'{"status":"execution-error","health":"untrusted-evidence","reason":"violations absent; a missing count is not a measured zero"}' '{"complexity_violations":0}'
	exit 0
fi
if [ "$V" = "invalid" ]; then

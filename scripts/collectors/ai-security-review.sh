#!/bin/sh
# Sentinel Shield collector — ai-security-review. Maps finding count -> ai_review_findings.
set -eu
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
# shellcheck source=scripts/lib/sentinel-shield-common.sh
. "$SCRIPT_DIR/../lib/sentinel-shield-common.sh"
TOOL="ai-security-review"
INPUT="reports/raw/ai-security-review.json"
while [ $# -gt 0 ]; do case "$1" in
  --input) INPUT="${2:?--input requires a value}"; shift 2 ;;
  --tool-name) TOOL="${2:?--tool-name requires a value}"; shift 2 ;;
  --producer-key) PRODUCER="${2:?--producer-key requires a value}"; shift 2 ;;
  -h|--help) echo "Usage: ai-security-review.sh [--input <path>] [--tool-name <name>] [--producer-key <key>]"; exit 0 ;;

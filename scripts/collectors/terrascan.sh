...
while [ $# -gt 0 ]; do case "$1" in
  ...
  -h|--help) echo "Usage: terrascan.sh [--input <path>] [--tool-name <name>] [--producer-key <key>]"; exit 0 ;;
  *) log_error "unknown argument: $1"; exit 2 ;;
esac; done
ss_collector_guard "$TOOL" "$INPUT"
jq -e '(type == "object" and has("results")) or . == {}' "$INPUT" >/dev/null 2>&1 \
	|| { log_warn "$TOOL: report has no .results (malformed/error output); status=execution-error (fail-closed)"; ss_emit_collector "$TOOL" "execution-error" '{"status":"execution-error"}' '{}'; exit 0; }

...
jq -e '(type == "array") or . == {}' "$INPUT" >/dev/null 2>&1 \
	|| { log_warn "$TOOL: report is not a JSON array (malformed/error output); status=execution-error (fail-closed)"; ss_emit_collector "$TOOL" "execution-error" '{"status":"execution-error"}' '{}'; exit 0; }

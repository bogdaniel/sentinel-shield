...
expect() {
  ...
  # MUST return the pass/fail status. Several assertions run inside `( VAR=x expect ... )`
  # subshells (to scope RUN_*/FAIL_* env), and a subshell's FAILS increment is discarded on
  # exit. Without a nonzero return the caller cannot tell the assertion failed, so those
  # checks could never fail the suite — exactly the test theatre this PR removes. The
  # subshell callers now propagate this status into the parent FAILS.
  if [ "$_rc" = "$_exp" ]; then pass "$_desc (exit $_rc)"; return 0; else fail "$_desc (expected $_exp, got $_rc)"; return 1; fi
}
...
( RUN_NAME=ci-docker expect 1 "F3 wrong workflow name rejected" --file "$EV" --verify-github ) || FAILS=$((FAILS + 1))
( RUN_EVENT=pull_request expect 1 "F3 PR run labeled push rejected (api event mismatch)" --file "$EV" --verify-github ) || FAILS=$((FAILS + 1))
( RUN_BRANCH=feature-x expect 1 "F3 non-default-branch run rejected" --file "$EV" --verify-github ) || FAILS=$((FAILS + 1))
( DEF_BRANCH=develop expect 1 "F3 default-branch mismatch rejected" --file "$EV" --verify-github ) || FAILS=$((FAILS + 1))
( RUN_REPO=org/other expect 1 "F3 repository.full_name mismatch rejected" --file "$EV" --verify-github ) || FAILS=$((FAILS + 1))
( FAIL_REPOMETA=1 expect 1 "F3 missing repository metadata rejected" --file "$EV" --verify-github ) || FAILS=$((FAILS + 1))
( FAIL_RUN=1 expect 1 "F3 nonexistent run rejected" --file "$EV" --verify-github ) || FAILS=$((FAILS + 1))

...
risk() {
	jq -n --argjson extra "$2" --arg t "$AR_TODAY" --arg s "$AR_SOON" '{version:"2", risks:[
		({ id:"AR-1", gate:"medium_vulnerabilities", scope:"finding",
		   owner:"sec@example.com", reason:"reviewed; compensating control",
		   created_at:$t, approved_at:$t, expires_at:$s, status:"approved" } + $extra) ]}' > "$1"
}
...
# ---------------------------------------------------------------------------
D=$(mkfix expired)
jq -n '{version:"2", risks:[{id:"AR-1", gate:"medium_vulnerabilities", scope:"finding", components:["lodash","symfony/http-kernel"], owner:"o", reason:"r", created_at:"1999-12-01", approved_at:"1999-12-01", expires_at:"2000-01-01", status:"approved"}]}' > "$D/ar.json"
...
D=$(mkfix pending)
jq -n '{version:"2", risks:[{id:"AR-1", gate:"medium_vulnerabilities", scope:"finding", components:["lodash","symfony/http-kernel"], owner:"o", reason:"r", created_at:"'"$AR_TODAY"'", expires_at:"'"$AR_SOON"'", status:"pending"}]}' > "$D/ar.json"
check "a PENDING record suppresses nothing" "$(enforce "$D" "$D/ar.json")" 1

D=$(mkfix noowner)
jq -n '{version:"2", risks:[{id:"AR-1", gate:"medium_vulnerabilities", scope:"finding", components:["lodash","symfony/http-kernel"], owner:"", reason:"r", approved_at:"'"$AR_TODAY"'", expires_at:"'"$AR_SOON"'", status:"approved"}]}' > "$D/ar.json"
check "an ownerless record is a schema violation (v2 rejects empty required values)" "$(enforce "$D" "$D/ar.json")" 2

D=$(mkfix ambiguous)
jq -n '{version:"2", risks:[{id:"AR-1", gate:"medium_vulnerabilities", scope:"finding", owner:"o", reason:"r", created_at:"'"$AR_TODAY"'", approved_at:"'"$AR_TODAY"'", expires_at:"'"$AR_SOON"'", status:"approved"}]}' > "$D/ar.json"
...
jq -n '{version:"2", risks:[{id:"AR-S", gate:"secrets", scope:"gate", owner:"o", reason:"r", created_at:"'"$AR_TODAY"'", approved_at:"'"$AR_TODAY"'", expires_at:"'"$AR_SOON"'", status:"approved"},
                            {id:"AR-1", gate:"medium_vulnerabilities", scope:"finding", components:["lodash","symfony/http-kernel"], owner:"o", reason:"r", created_at:"'"$AR_TODAY"'", approved_at:"'"$AR_TODAY"'", expires_at:"'"$AR_SOON"'", status:"approved"}]}' > "$D/ar.json"
...
# ---------------------------------------------------------------------------
D=$(mkfix broad)
jq -n '{version:"2", risks:[{id:"AR-B", gate:"medium_vulnerabilities", scope:"gate", owner:"o", reason:"legacy broad record", created_at:"'"$AR_TODAY"'", approved_at:"'"$AR_TODAY"'", expires_at:"'"$AR_SOON"'", status:"approved"}]}' > "$D/ar.json"
...
D=$(mkfix norecord)
jq -n '{version:"2", risks:[]}' > "$D/ar.json"
...
jq -n '{version:"2", risks:[{id:"AR-D", gate:"unsafe_docker", scope:"finding", rule_id:"DL3018", files:["Dockerfile"], owner:"o", reason:"r", created_at:"'"$AR_TODAY"'", approved_at:"'"$AR_TODAY"'", expires_at:"'"$AR_SOON"'", status:"approved"}]}' > "$D/ar.json"
check "unsafe_docker finding scope still accepts a matched finding" "$(enforce "$D" "$D/ar.json")" 0
jq -n '{version:"2", risks:[{id:"AR-D", gate:"unsafe_docker", scope:"finding", rule_id:"DL3008", files:["Dockerfile"], owner:"o", reason:"r", created_at:"'"$AR_TODAY"'", approved_at:"'"$AR_TODAY"'", expires_at:"'"$AR_SOON"'", status:"approved"}]}' > "$D/ar2.json"
...
for _gate in unsafe_docker medium_vulnerabilities; do
	...
	jq -n --arg g "$_gate" '{version:"2", risks:[{id:"oops", gate:$g, scope:"finding",
	...
	if grep -q 'ambiguous' "$D/log"; then
		...
		fail "  but it was not reported as ambiguous: $(grep -iE 'error|ambiguous' "$D/log" | head -2)"
	fi
	# A record that DOES constrain still works — the fix must not disable finding scope.
	jq -n --arg g "$_gate" '{version:"2", risks:[{id:"real", gate:$g, scope:"finding",
		rule_ids:["DL3018"], files:[], components:[], fingerprints:[],
		owner:"o", reason:"r", created_at:"'"$AR_TODAY"'", approved_at:"'"$AR_TODAY"'", expires_at:"'"$AR_SOON"'", status:"approved"}]}' > "$D/ar2.json"
	sh "$ROOT/scripts/enforce-gates.sh" --gates-env "$D/sentinel-shield-gates.env" \
